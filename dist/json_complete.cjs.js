...
var compressValues = function compressValues(key, value, types) {
  ...
  if (types[key]._compressionType === 1) {
    ...
    var characters = value.join(',').replace(/E/g, 'e').split('');
    var previous = 0;
    var encoded = [];

    for (var c = 0; c < characters.length; c += 1) {
      var index = numberEncoding.indexOf(characters[c]);
...
var genEqualitySimpleType = function genEqualitySimpleType(value) {
  return {
    _identify: function _identify(comparedTo) {
...
var decompressValues = function decompressValues(key, value, types) {
  ...
  if (types[key]._compressionType === 1) {
    var decoded = []; // Decode two characters per loop

    var valueLength = value.length;

    for (var i = 0; i < valueLength - 1; i += 2) {
      decoded = decoded.concat([numberEncoding[(fromBase(value[i], alphabet) & 60) >>> 2], numberEncoding[(fromBase(value[i], alphabet) & 3) << 2 | (fromBase(value[i + 1], alphabet) & 48) >> 4], numberEncoding[fromBase(value[i + 1], alphabet) & 15]]);
    } // Account for uneven numbers of characters


    if (valueLength % 2 !== 0) {
      decoded = decoded.concat([numberEncoding[fromBase(value[valueLength - 1], alphabet) >>> 2]]);
